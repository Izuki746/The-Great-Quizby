...
export function ProfileView(user, myQuizzes) {
  // Dynamic generate HTML of history record
  const historyHtml = user.quizHistory && user.quizHistory.length > 0 
    ? user.quizHistory.slice().reverse().map(result => {
        ...
        // Use different color for score due to accuracy
        const colorClass = acc >= 80 ? 'text-emerald-400 bg-emerald-400/10' : 
                           acc >= 50 ? 'text-yellow-400 bg-yellow-400/10' : 
          ...
          <tr>
             <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                   ...
                   <div>
                      <p class="font-bold text-white">${result.topic}</p>
                      <p class="text-xs text-slate-500">${result.totalQuestions} Questions</p>
                   </div>
                </div>
             </td>
             <td class="px-6 py-4">
                <span class="${colorClass} font-bold px-2 py-1 rounded">
                  ${acc}%
                </span>
             </td>
             <td class="px-6 py-4">${dateStr}</td>
             <td class="px-6 py-4 text-right">
                <span class="text-primary font-bold">+${result.score} pts</span>
             </td>
          </tr>
        `;
      }).join('') 
          ...
          <div class="relative z-10 flex flex-col items-center py-12 px-4">
             ...
             <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-10 w-full max-w-[960px] justify-items-center">
                ...
                <div class="glass-card rounded-xl p-6 relative overflow-hidden group text-center w-full max-w-sm">
                   ...
                   <p class="text-slate-400 text-xs font-bold uppercase tracking-wider">Average Accuracy</p>
